...
RUN mkdir -p /usr/src/fabspace

WORKDIR /usr/src/fabspace
...
COPY . /usr/src/fabspace

RUN chmod +x /usr/src/fabspace/tasks.py \
    && chmod +x /usr/src/fabspace/entrypoint.sh

# app-specific requirements
RUN pip install --upgrade --no-cache-dir --src /usr/src -r requirements.txt
RUN pip install --upgrade -e .

ENTRYPOINT ["/usr/src/fabspace/entrypoint.sh"]